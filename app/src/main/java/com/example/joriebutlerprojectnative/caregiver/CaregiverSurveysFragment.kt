...
/**
 * A simple [Fragment] subclass. Use the [CaregiverSurveysFragment.newInstance] factory method to
...
class CaregiverSurveysFragment : Fragment() {
  ...
  override fun onCreateView(
    ...
    binding.buttonCBS.setOnClickListener {
      parentFragmentManager
        .beginTransaction()
        .replace(R.id.fragmentContainerView2, CaregiverBurdenScaleFragment())
        .commit()
    }
    binding.buttonActivitiesOfDailyLifeScale.setOnClickListener {
      parentFragmentManager
        .beginTransaction()
        .replace(R.id.fragmentContainerView2, ActivitiesOfDailyLifeScaleFragment())
        .commit()
    }
    binding.buttonInstrumentalActivitiesOfDailyLifeScale.setOnClickListener {
      parentFragmentManager
        .beginTransaction()
        .replace(R.id.fragmentContainerView2, InstrumentalActivitiesOfDailyLifeScaleFragment())
        .commit()
    }
    binding.buttonSRI.setOnClickListener {
      parentFragmentManager
        .beginTransaction()
        .replace(R.id.fragmentContainerView2, SocialRelationshipIndexFragment())
        .commit()
    }

    val caregiverSharedPref =
      requireActivity()
